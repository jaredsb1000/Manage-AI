...
class SpoofLibrary:
    def get_character(self):
        ...
        characters = [
            ...
            "Alien Survivor", "Speedster Hero", "Star Soldier", "Iron Knight", "Green Giant Monster",
            "Ice Sorceress", "Snow Minions", "Wooden Puppet", "Space Cowboy", "Valkyrie Pilot", "Blue Alien Cat",
            "Submarine Captain", "Pink Panther", "Cartoon Rabbit", "Duck Hunter", "Road Runner", "Coyote Hunter",
            "Fat Cat", "Beagle Dog", "Yellow Family", "Space Mutants", "Samurai Warrior", "Ed, Edd, Eddy",
            "Johnny Bravo", "Courage Dog", "Boy Genius", "Dee Dee", "Mandark", "Powerpuff Girls",
            "Professor", "Evil Genius", "Number 4", "Sector V Operatives", "Numbuh 4", "Wallaby Foster"
        ]
...
class CharacterForge:
    def generate_data(self, rarity):
        ...
        skill_pool = [
            "Fireball", "Data Mining", "Stealth", "Hacking", 
...
# --- ART GENERATOR ---
class ArtGenerator:
    def create_spoof_art(self, character_name, rarity, premium_mode=False):
        import requests
        
        # 1. Define Style
        if premium_mode:
            # POWER MODE: Use Expert Prompts
            style_modifier = random.choice([
                "Hyper-realistic, 8K resolution, volumetric lighting, Octane render, Unreal Engine 5 style",
                "Cinematic masterpiece, intricate details, anamorphic lens flare, film grain, dramatic lighting",
                "Biomechanical art, schematic overlays, glowing blue core, cybernetic implants, tech noir",
                "Abstract surrealism, dreamscape, melting clocks, impossible geometry, Salvador Dali style"
            ])
            print("-> [ART] **POWER MODE ACTIVATED** - Generating Masterpiece...")
        else:
            # STANDARD MODE
        ...
        remote_url = f"https://pollinations.ai/p/{safe_prompt}?width=600&height=400&seed={seed}&nologo=true&enhance=true"
        ...
        try:
            ...
            if response.status_code == 200:
                # Make sure to 'images' folder exists
                ...
                # Return ROOT path 'images/filename'
                return f"images/{seed}.jpg"
            else:
...
Only {price}.00 for receipt + link.

Click to link to view/buy! 👇
...
class LemonManager:
    ...
    def create_receipt_product(self, character, art_url, sale_price, physical_price, rarity, stats, skills):
        ...
        if self.store_id == "PASTE_YOUR_LEMON_STORE_ID_HERE":
            ...
            # Return a fake link so the script continues
...
class Manage:
    def __init__(self):
        ...
        self.forge = CharacterForge() # NEW: Soul Binder
        ...
        print(f"Manage v18.0 Online. Omniscient Manager Active.")
    ...
    def daily_grind(self):
        ...
        # 4.5: Generate RPG Stats (The Soul Binder)
        print("-" * 40)
        print("PHASE 2.5: BINDING SOUL")
        stats, skills = self.forge.generate_data(rarity)
        print(f"-> [GAME] Stats: STR: {stats['str']} | DEX: {stats['dex']} | INT: {stats['int']}")
        print(f"-> [GAME] Skills: {', '.join(skills)}")
        ...
        # Passing stats and skills to the receipt generation for RPG data visibility
        link = self.hands_lemon.create_receipt_product(character, art_url, sale_price, physical_price, rarity, stats, skills)
        ...
        nexus_item = {
            ...
            "status": "Virtual",
            "social_kit": hype_kit,
            "game_stats": stats, # NEW: RPG Stats Attached
            "skills": skills         # NEW: RPG Skills Attached
        }
